...
start.all <- matrix(rnorm(dim*N, mean = 0, sd = 100), nrow = N, ncol = dim)
tol = 1e-3
...
for (j in 1:N)
{
  ...
  start.time <- Sys.time()
  fp <- BFGS(par = start, A=A, b=b, L=L, fixptfn = update, objfn = objective, control = list(tol = tol, objfn.inc = 1, maxiter = 5e4))
  end.time <- Sys.time()
...
for (j in 1:N)
{
  ...
  start.time <- Sys.time()
  fp <- LBFGS(par = start, A=A, b=b, L=L, fixptfn = update, objfn = objective, control = list(tol = tol, objfn.inc = 1, maxiter = 5e4))
  end.time <- Sys.time()
...
for (j in 1:N){
  ...
  start.time <- Sys.time()
  fp <- squarem(start, A=A, b=b, L=L, fixptfn = update, objfn = objective, control = list(K=1, tol = tol, method = 1, maxiter = 5e4))
  end.time <- Sys.time()
...
for (j in 1:N){
  ...
  start.time <- Sys.time()
  fp <- squarem(start, A=A, b=b, L=L, fixptfn = update, objfn = objective, control = list(K=1, tol = tol, method = 2, maxiter = 5e4))
  end.time <- Sys.time()
...
for (j in 1:N){
  ...
  start.time <- Sys.time()
  fp <- squarem(start, A=A, b=b, L=L, fixptfn = update, objfn = objective, control = list(K=1, tol = tol, method = 3, maxiter = 5e4))
  end.time <- Sys.time()
...
pdf(file = "Out/scatter_plot_quad_s100.pdf")

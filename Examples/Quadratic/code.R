...
for (i in 1:N)
{
  ...
  fp <- LBQN(par = start, A=A, a=a, L=L, fixptfn = update, objfn = objective,
              control = list(tol = tol, maxiter = 1e5, m = 2))
  end.time <- Sys.time()
...
save(time_mm, time_bqn1, time_bqn2, time_bqn3, time_lbqn, time_sq1, time_sq2, time_sq3, time_zal, time_zal2, time_dar,
     ...
     obj_mm, obj_bqn1, obj_bqn2, obj_bqn3, obj_lbqn, obj_sq1, obj_sq2, obj_sq3, obj_zal, obj_zal2, obj_dar, file = "Out/quad-objects_sq1e3.Rdata")
